...
public class AddNewDataClass {
    ...
    /**
     * this method is used to edit  data
     * @param data a new record
     */

    public void editData(AddNewData editedData) {
        int index = records.indexOf(editedData);
        if (index != -1) {
            records.set(index, editedData);
        } else {
            throw new IllegalArgumentException("Data with the specified ID does not exist.");
        }
    }
}
